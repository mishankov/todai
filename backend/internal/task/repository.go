...
// Create inserts an active top-level Inbox task.
func (r *Repository) Create(ctx context.Context, userID, title string) (Task, error) {
	var created Task
	err := r.db.GetContext(ctx, &created, `
		INSERT INTO tasks (
			id, user_id, title, status, priority, position, version,
			created_at, updated_at, last_modified_by
		)
		SELECT
			$1::VARCHAR, $2::VARCHAR, $3::TEXT, 'active', 0,
			COALESCE(MAX(position), 0) + 1024, 1,
			CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $2::VARCHAR
		FROM tasks
		WHERE user_id = $2::VARCHAR AND project_id IS NULL AND parent_id IS NULL
		RETURNING `+taskColumns,
		uuid.NewString(), userID, title,
	)
	if err != nil {
...
func (r *Repository) Update(
	...
	err := r.db.GetContext(ctx, &updated, `
		...
		SET title = CASE WHEN $4::BOOLEAN THEN $5::TEXT ELSE title END,
			...
			priority = CASE WHEN $8::BOOLEAN THEN $9::SMALLINT ELSE priority END,
			due_at = CASE WHEN $10::BOOLEAN THEN $11::TIMESTAMPTZ ELSE due_at END,
			due_timezone = CASE WHEN $12::BOOLEAN THEN $13::TEXT ELSE due_timezone END,
			version = version + 1,
