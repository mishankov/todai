...
ALTER TABLE projects
	ADD COLUMN color_theme VARCHAR(20) NOT NULL DEFAULT 'sage',
	ADD COLUMN agent_model VARCHAR(255) NOT NULL DEFAULT '',
	ADD COLUMN agent_thinking_effort VARCHAR(20) NOT NULL DEFAULT 'medium',
	ADD CONSTRAINT projects_color_theme_valid CHECK (
		color_theme IN ('sage', 'ocean', 'plum', 'sand', 'rose', 'graphite')
	),
	ADD CONSTRAINT projects_agent_thinking_effort_valid CHECK (
...
BEGIN
	IF TO_REGCLASS('user_settings') IS NOT NULL THEN
		...
		WHERE settings.user_id = project.user_id;
	END IF;
END
$$;
...
ALTER TABLE projects
	DROP CONSTRAINT projects_agent_thinking_effort_valid,
	DROP CONSTRAINT projects_color_theme_valid,
